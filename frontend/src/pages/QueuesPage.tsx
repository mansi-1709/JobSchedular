    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {queues.map(queue => (
          <div
            key={queue.id}
            className="p-6 rounded-2xl bg-surface-elevated/90 border border-slate-200 dark:border-white/10 hover:border-indigo-500/40 transition-all duration-300 shadow-md flex flex-col justify-between group hover:-translate-y-1"
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <Badge status={queue.status} />
                <span className="text-[11px] font-semibold text-indigo-700 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 px-2.5 py-0.5 rounded-md border border-indigo-200 dark:border-indigo-800/40">
              ...
              <p className="text-xs text-slate-500 dark:text-gray-400 mt-1 min-h-[32px] line-clamp-2">
                {queue.description || 'General async task processing pipeline.'}
              </p>

              {/* Specs Grid */}
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-white/10 grid grid-cols-2 gap-3 text-xs">
                <div className="flex items-center gap-2 text-slate-600 dark:text-gray-300">
                  <Zap className="w-4 h-4 text-emerald-500" />
                  <span>
                    Limit: <strong className="text-slate-900 dark:text-white">{queue.concurrencyLimit}</strong> workers
                  </span>
                </div>
                <div className="flex items-center gap-2 text-slate-600 dark:text-gray-300">
                  <Repeat className="w-4 h-4 text-amber-500" />
                  <span>
                    Retries: <strong className="text-slate-900 dark:text-white">{queue.maxRetries}</strong>
                  </span>
                </div>
              </div>
            </div>

            <div className="mt-5 pt-4 border-t border-gray-100 dark:border-white/10 flex items-center justify-between gap-2.5">
      ...
      {queues.length === 0 && !isCreating && (
        <div className="text-center py-16 bg-surface-elevated/40 rounded-2xl border border-dashed border-slate-300 dark:border-white/10">
          <Layers className="w-12 h-12 mx-auto text-gray-400 mb-3" />
          <h3 className="text-slate-900 dark:text-white font-bold mb-1">No queues configured</h3>
          <p className="text-xs text-slate-500 dark:text-gray-400 mb-4">Create your first processing queue to begin routing background jobs.</p>
